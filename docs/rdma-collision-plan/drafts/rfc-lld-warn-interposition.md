...
- **Route A (interposition capture), reproduced:** the `-Bsymbolic` trigger is absent from public wheels (DF_SYMBOLIC = 0 / 366 libs — it lives in monorepo static-link builds), but `import torch` runs `ctypes.CDLL(libtorch_global_deps.so, RTLD_GLOBAL)`. An `LD_DEBUG` trace shows faiss's traced OpenMP references binding faiss's own libgomp when faiss is imported alone, and all of them rebinding to torch's copy once torch is imported first (`route_A_hit` confirmed). And under a real workload, **206 duplicated compute symbols show a confirmed runtime split** — 204 of them faiss's embedded BLAS versus the OpenBLAS that numpy/scipy bundle. A stock-PyPI reproduction of the incident's mechanism.
