...
- **Route B — scope partition**: `RTLD_LOCAL` + vendored duplication *alone* — no self-binding, no interposition. Module X binds copy 1, module Y binds copy 2, each runs its own state. **Confirmed live (measured):** import faiss + scikit-learn + torch into one process (219 mapped `.so`) and `/proc/self/maps` shows **2 distinct libgomp builds, 2 libgfortran, 2 libquadmath** — different sha256, both constructors run. numpy + scipy alone → 2 libgfortran + 2 libquadmath. The ecosystem already knows this as the "multiple OpenMP runtimes" problem and ships a runtime kill-switch (Intel's `KMP_DUPLICATE_LIB_OK` → "OMP: Error #15… can cause incorrect results") — a *named, loud* instance of the class nobody connects to Route A. (Honest nuance to state: 3 wheels bundle libgomp but only 2 distinct builds map — faiss & sklearn share auditwheel's content-hashed copy; torch ships its own.)
- **Route A, the reproducible public hit (the gem):** the `-Bsymbolic` trigger is absent from public wheels (DF_SYMBOLIC = 0 across 366 libs — it lives in monorepo land), **but** torch's `__init__.py` does `ctypes.CDLL(libtorch_global_deps.so, RTLD_GLOBAL)`, promoting torch's OpenMP into the global scope. Controlled `LD_DEBUG` experiment: faiss imported alone binds its own libgomp; **import torch first and the same faiss OpenMP references rebind to torch's copy** (all traced references flip; `route_A_hit` confirmed). And under a real workload, **206 duplicated compute symbols show a confirmed runtime split** — 204 of them faiss's embedded BLAS versus the OpenBLAS numpy/scipy bundle (`tier3_workload.json`). A concrete, reproducible, stock-PyPI demonstration of the incident's mechanism in miniature. (Benign in *effect* here only because OpenMP's duplicated pool state is functionally redundant — the exact condition Intel's Error #15 guards against; the structural split is real.)
- The "taxes table" (verified): `KMP_DUPLICATE_LIB_OK` (Intel's shipping Route-B detector + kill-switch), auditwheel content-hash soname mangling (sold as clash-avoidance — actually *enables* Route B by letting copies coexist isolated instead of erroring), `rename_symbols.sh` (monorepo Route-A avoidance), conda single-copy-per-env / nomkl (the design that avoids Route B entirely).
- Tier-0, collapsed to "N copies of library L" (raw 8,181 is nm-level, ignores scope): libgomp / libgfortran / libquadmath each bundled by 3 wheels, 2 distinct builds, 2 mapped at runtime.
- The finding in one line: *the incident's exact trigger (`-Bsymbolic` self-binding) is absent from the public wheel channel because it lives in monorepo static-link land you can't scan from PyPI — yet the same disease is live in every ML process via Route B, and its Route-A variant is one `RTLD_GLOBAL` `ctypes.CDLL` away, which is exactly what `import torch` does.*
Artifacts: `demo/rdma-symbol-collision/survey/full/` — routeB_copycount.py, routeA_scope_promotion.py, workload_capture.py; results/routeB_copycount.json, results/routeA_scope_promotion.json, results/routeB_maps/.
