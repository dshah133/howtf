...
The incident reached the same split through a different door. Its double inclusion — the one the root cause named — had two copies with names and a history. Copy one was bundled: the in-house collective library was compiled into the binary's big composed native library — the "hold that thought" from section 1 — and it brought its verbs with it, libibverbs and the mlx5 provider, statically linked, symbols private. Private is the load-bearing word. Statically linked verbs whose symbols stay local never appear in the process's dynamic symbol tables; every verbs call the in-house library made was resolved at build time, inside itself, end to end. rdma-core supports exactly this shape — a static build [compiles the dlopen loader out entirely](https://github.com/linux-rdma/rdma-core/blob/master/libibverbs/dynamic_driver.c) and pulls providers in through [`ibv_static_providers()`](https://github.com/linux-rdma/rdma-core/blob/master/libibverbs/static_driver.c). Copy two was the system's: the same stack reached at runtime through `dlopen` — how NCCL gets its verbs, deliberately [loading the system `libibverbs`](https://github.com/NVIDIA/nccl/blob/master/src/misc/ibvsymbols.cc) instead of linking it, and how that libibverbs in turn finds its hardware providers, [dlopening the driver](https://github.com/linux-rdma/rdma-core/blob/master/libibverbs/dynamic_driver.c) named in its config. Two instances, two consumers, zero contact — a private copy and a public copy have nothing to fight over. It worked.

Then a build change made the in-house library's verbs dynamically linked instead. (I'll be honest about the edge of my recall: I no longer have the diff. What I have is the before, the after, and Part 1's rules, which leave one road between them — that road is what I'm describing.) A dynamically linked libibverbs exports its symbols at default visibility into the global dynamic scope — the same namespace where the system copy's identical names already lived. From that moment, which instance answered any given verbs call stopped being the in-house library's internal affair and became a per-reference decision made by the loader's lookup order. Interposition, which had had nothing to grab, applied.

Why that surfaces as an empty device list rather than a crash is down to how rdma-core keeps its books. Every instance of libibverbs carries its own file-static state: the device list `ibv_get_device_list()` hands back lives in [a static inside `device.c`](https://github.com/linux-rdma/rdma-core/blob/master/libibverbs/device.c), and the driver registry it is built from is [a static `driver_list` inside `init.c`](https://github.com/linux-rdma/rdma-core/blob/master/libibverbs/init.c). Providers are not handed over; they announce themselves: libmlx5 runs an ELF constructor — the [`PROVIDER_DRIVER` macro](https://github.com/linux-rdma/rdma-core/blob/master/libibverbs/driver.h) — that calls `verbs_register_driver()`, appending the mlx5 driver to the registry of whichever libibverbs instance that call binds to. Discovery then walks sysfs and matches what the kernel reports against its own instance's registry, and a device that matches no registered driver is silently dropped from the result (the warning that would have named the problem hides behind an `IBV_SHOW_WARNINGS` environment variable). The whole outcome, in other words, hangs on two symbol bindings: which instance received the mlx5 registration, and which instance served the discovery call. Before the change, both resolved inside the private copy — one instance, one registry, one truth. After it, with two instances' worth of identical names in one scope, the bindings diverged: registration filled one instance's registry, and the in-house library's discovery read the other's, which nothing had filled. Every device the kernel reported matched no driver. Zero devices. `No IB devices found` — on a machine where NCCL, consistent on the copy it had dlopened, could see them all. (The dlopen road is also a preview — it is exactly the runtime-scope machinery that returns in section 7 as Route B.)
...
In the incident, both rungs got used, in the order SEV pressure dictates. The immediate mitigation was to make the in-house collective library opt-in: binaries that didn't need MTIA stopped pulling it in, and the double inclusion simply stopped happening in the common path — the bug defused by removing one of the two copies from most processes, not by fixing the collision. The principled fix came after: put libibverbs and libmlx5 back to being statically linked, symbols private again — the self-contained shape from section 3, the one that had never fought with the system copy. In the fix ladder's terms it removes the duplicate from the scope where discovery binds, the first rung's spirit — shipped in production before the reproducer existed to validate it.
...
**Route A — interposition capture** is the reproducer's shape: a duplicate strong symbol, a self-binding library, and an interposing module sharing one symbol scope. The incident is its close cousin — the same global-scope collision, but with the second instance arriving by `dlopen` rather than the link line, one foot in each route. **Route B — scope partition** needs neither self-binding nor interposition. If two modules are loaded into separate local scopes — `RTLD_LOCAL`, the default for every `dlopen`, which is how Python loads extension modules — and each carries its own vendored copy of a library, then each side binds its own copy and runs its own state. Same disease, two live copies of one library's state, reached without any special flag at all.
