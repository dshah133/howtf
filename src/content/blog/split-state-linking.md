...
Some context, because the shape of the build matters later. This was at Meta. The binaries were application training binaries composed by Buck, with PyTorch built in-house and statically linked — hermetic builds and fast startup are worth a great deal at that scale. The public Buck documentation spells out the backbone of what "composed by Buck" means for a Python binary: a training program pulls in an enormous number of native extension modules, and Buck's [omnibus](https://buck.build/javadoc/com/facebook/buck/cxx/Omnibus.html) strategy keeps that manageable by statically linking most of the native code "into a single giant shared library" — a `libomnibus.so` — leaving only the extensions Python imports directly as separate .so's. When that merge can't be clean — prebuilt libraries that can't be relinked, colliding symbols, or a binary pushing past the 2 GiB relocation barrier — the composition falls back to separate copies, or gets carved into link groups to keep it linkable at all. Meta had even written down the hazard, in the Android sibling of this exact machinery, back in [2018](https://engineering.fb.com/2018/01/23/android/android-native-library-merging/): native-library merging "works great, as long as there are no common symbols between the libraries being merged." (Nor is it private knowledge that this gets sharp around torch — Meta's own tracker has [buck2 #62](https://github.com/facebook/buck2/issues/62), libomnibus turning a symbol undefined that libtorch_cpu, libc10, and libtorch_python all keep weak.) Merge what you can, duplicate what you can't: that is the world in which the same native library can end up in one binary twice. Torch was one ingredient; the final artifact was each application's own training binary. And some of those binaries, depending on what they trained on, carried support for MTIA, Meta's own accelerator — provided by an in-house collective-communication library that discovers its devices through RDMA the same way NCCL does.
