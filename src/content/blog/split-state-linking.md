...
**Route A — interposition capture** is the reproducer's shape: a duplicate strong symbol, a self-binding library, and an interposing module sharing one symbol scope. The incident is its close cousin, one foot in each route: the captured reference was the provider's registration import, interposed through the global scope — Route A's move — while the victim was a consumer pinned by `dlsym` to an `RTLD_LOCAL` copy the capture could never fill, which is Route B's scope machinery doing the isolating. **Route B — scope partition** needs neither self-binding nor interposition. If two modules are loaded into separate local scopes — `RTLD_LOCAL`, the default for every `dlopen`, which is how Python loads extension modules — and each carries its own vendored copy of a library, then each side binds its own copy and runs its own state. Same disease, two live copies of one library's state, reached without any special flag at all.
